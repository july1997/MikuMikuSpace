...
void Character::playAnime()
{
    if (!dance)
    {
        ...
        //経過秒数からアニメーションの加算値を求める
        nowtime = GetNowCount();

        if (oldtime != 0)
        {
            int ptime = nowtime - oldtime;
            passagetime = (float)ptime / 1000.f * 30.f;
            time += passagetime;
        }
...
void Character::stateManager()
{
    ...
    else
    {
        ...
        if ((!_forward) && (state == "run" || state == "run_l" || state == "run_s"))
        {
            state = "run_e";
            timer2 = 101.f;
            inputstate = state;
        ...
        if (state == "run" && state.find("dash") == std::string::npos && shifttimer > 30)
        {
            state = "dash_s";
            inputstate = state;
        }
...
void Character::animeControl()
{
    ...
    if (state == "walk")
    {
        ...
        {
            if (switchAnime(nowAni, 2, timer2, 0.1))
            {
                time = timer2;
                timer2 = 0;
            }
    ...
    if (state == "run_l")
    {
        if (switchAnime(nowAni, 5, time + 5, 0.1f))
        {
            state = "run";
            time += 5;
        }
    ...
    if (state == "run_e")
    {
        if (deley == 0)
        {
            if (switchAnime(nowAni, 6, timer2, 0.1f))
            {
                deley = 1;
                time = timer2;
                timer2 = 0;
    ...
    if (state == "run_f")
    {
        if (switchAnime(nowAni, 0, timer2, 0.01f))
        {
            state = "stand";
            time = timer2;
            timer2 = 0;
    ...
    {
        if (switchAnime(nowAni, 7, timer2, 0.05f))
        {
            state = "run_walk";
            time = timer2;
            timer2 = 0;
    ...
    if (state == "dash_s")
    {
        if (switchAnime(nowAni, 9, timer2, 0.05f))
        {
            state = "dash";
            time = timer2;
            timer2 = 0;
    ...
    if (state == "dash_e")
    {
        if (switchAnime(nowAni, 5, timer2, 0.05f))
        {
            state = "run";
            time = timer2;
            timer2 = 0;
